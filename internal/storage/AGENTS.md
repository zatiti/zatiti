...
Own the SQLite writer, consistent reads, transaction Unit, migrations and atomic event outbox.

Write scope: **`internal/storage/` only**, excluding this generated AGENTS.md. Go package name: `storage`. Ownership kind: foundation; integration wave: 0.
...
Local proving focus: Rollback and fault injection between state/outbox writes, concurrent serialized writers, busy deadline, read snapshot consistency, callback called once, migration digest mismatch, restart generation, valid WAL backup/restore.
