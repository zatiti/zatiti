...
// Service assembly. Memory constructs as New(deps), registers every owned
// operation through the local bind (the frozen registry.Bind contract,
// mirrored locally because memory may only import the shared contract), and
// dispatches strictly. Clock, identity source and ports are required:
// dispatch resolves policy, configuration, artifacts, effects and execution
// through peers, and a memory module without them could only invent answers.

// ownerName is the domain owner identity used in descriptors, migrations and
// events.
const ownerName = "memory"
...
const (
	...
	opBindingGet     = "memory.binding.get"
	opBindingList    = "memory.binding.list"
	...
	opArtifactsPublish   = "_artifacts.publish"
	opEffectsPrepare     = "_effects.prepare"
	opEffectsAdmit       = "_effects.admit"
	opExecutionJobCreate = "_execution.job.create"
	opExecutionJobRecord = "_execution.job.record"
)
...
var opMetas = []opMeta{
	...
	{id: opBootstrap, visibility: "internal", mode: "mutation",
...
// Service is the memory domain owner: scoped memory bindings, governed
// Serenity jobs, promotion/retraction lineage and reconciliation obligations.
type Service struct {
...
// New constructs the memory owner. It never queries peers, touches storage
// or starts goroutines; all runtime coupling arrives through deps.
func New(deps contract.Dependencies) (*Service, error) {
	if deps.Clock == nil {
		return nil, &contract.Fault{Code: contract.CodeInternalError, Message: "memory requires a clock"}
	}
	if deps.IDs == nil {
		return nil, &contract.Fault{Code: contract.CodeInternalError, Message: "memory requires an identity source"}
	}
	if deps.Ports == nil {
		return nil, &contract.Fault{Code: contract.CodeInternalError, Message: "memory requires a ports dependency"}
	}
	s := &Service{
		deps:     deps,
...
func (s *Service) assemble() error {
	...
	for _, m := range opMetas {
		...
		if !ok {
			return fmt.Errorf("memory: operation %s has no wire schemas", m.id)
		}
		...
		if completion, ok := completionSchemas[m.id]; ok {
			...
			if err != nil {
				return fmt.Errorf("memory: operation %s completion schema: %w", m.id, err)
			}
		...
		if err != nil {
			return fmt.Errorf("memory: bind %s: %w", m.id, err)
		}
		s.catalog[m.id] = d
		s.handlers[m.id] = h
...
func (s *Service) bindHandler(d contract.Descriptor) (contract.Handler, error) {
	...
	case opBootstrap:
		return bind(d, s.handleBootstrap)
	case opManifest:
	...
	case opBindingArchive:
		return bind(d, s.handleBindingArchive)
	case opBindingCreate:
		return bind(d, s.handleBindingCreate)
	case opBindingGet:
		return bind(d, s.handleBindingGet)
	case opBindingList:
		return bind(d, s.handleBindingList)
	case opBindingUpdate:
		return bind(d, s.handleBindingUpdate)
	case opInspect:
	...
	case opRecall:
		return bind(d, s.handleRecall)
	case opRemember:
		return bind(d, s.handleRemember)
	case opRetract:
		return bind(d, s.handleRetract)
	default:
...
func canonicalJSON(v any) (json.RawMessage, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("memory: encode: %w", err)
	}
	canonical, err := contract.Canonicalize(raw)
	if err != nil {
		return nil, fmt.Errorf("memory: canonicalize: %w", err)
	}
