...
const (
	eventArtifactPublished = "artifacts.artifact.published"
	eventArtifactFaulted   = "artifacts.artifact.faulted"
	eventUploadBegun       = "artifacts.upload.begun"
	eventUploadChunked     = "artifacts.upload.chunked"
	eventUploadFinished    = "artifacts.upload.finished"
	eventUploadCancelled   = "artifacts.upload.cancelled"
)
...
func emitTransition(ctx context.Context, unit contract.Unit, kind string, resourceID contract.ID, version contract.Version, data json.RawMessage) error {
	if err := unit.Emit(ctx, contract.Event{
		...
		return fmt.Errorf("artifacts: emit %s: %w", kind, err)
	}
	return nil
}
...
func (s *Service) cursorKeyLocked() ([]byte, error) {
	if len(s.cursorKey) == 0 {
		...
		if _, err := rand.Read(key); err != nil {
			return nil, fmt.Errorf("artifacts: mint cursor key: %w", err)
		}
		s.cursorKey = key
	}
...
func (s *Service) mintCursor(op string, unit contract.Unit, filter any, lastCreated time.Time, lastID contract.ID, now time.Time) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("artifacts: bind cursor filter: %w", err)
	}
...
func (s *Service) readCursor(op string, unit contract.Unit, filter any, raw string) (createdAt time.Time, lastID contract.ID, err error) {
	...
	filterJSON, err := json.Marshal(filter)
	if err != nil {
		return time.Time{}, "", fmt.Errorf("artifacts: bind cursor filter: %w", err)
	}
