...
func cursorExpired(format string, args ...any) *contract.Fault {
	f := fault(contract.CodeCursorExpired, format, args...)
...
// blobFault maps a blob-store failure onto the shared fault vocabulary.
// The platform byte store speaks coded, redacted errors: failures carrying
// a contract.Fault pass through, coded errors (controller_unavailable under
// disk pressure, artifact_fault for missing or corrupt bytes) keep their
// code, and anything else fails closed as artifact_fault without leaking
// the underlying message. Metadata messages never contain local paths.
func blobFault(err error) *contract.Fault {
	var f *contract.Fault
	if asFault(err, &f) && f != nil {
		return f
	}
	if coder, ok := err.(interface{ Code() string }); ok {
		switch code := coder.Code(); code {
		case contract.CodeControllerUnavailable:
			return &contract.Fault{
				Code:      code,
				Message:   "artifact staging is paused until storage pressure clears",
				Retryable: true,
			}
		case contract.CodeArtifactFault, contract.CodeNotFound, contract.CodeInvalidInput,
			contract.CodePermissionDenied, contract.CodePrerequisiteMissing, contract.CodeCapabilityUnsupported:
			return &contract.Fault{Code: code, Message: "artifact byte store rejected the request"}
		default:
			return &contract.Fault{Code: contract.CodeInternalError, Message: "artifact byte store failed"}
		}
	}
	return artifactFault("artifact byte store operation failed")
}
