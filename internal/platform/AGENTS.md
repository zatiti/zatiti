# Implementation assignment: `internal/platform`
...
Own OS locks, protected local configuration, secret custody, encrypted blob files and client secure storage.

Write scope: **`internal/platform/` only**, excluding this generated AGENTS.md. Go package name: `platform`. Ownership kind: foundation; integration wave: 0.
...
Local proving focus: Competing process lock, permissions, interrupted stage/publish, encrypted roundtrip/tamper, symlink escape, unavailable key store, redacted errors, disk-pressure refusal, secret absence from child environment. Rev9: fixed installed master-key custody, missing/locked/ACL-refused Keychain and native signed-client continuity.
