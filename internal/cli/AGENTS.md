# Implementation assignment: `internal/cli`
...
Own Cobra command generation, structured input, deterministic JSON output and process exit mapping.

Write scope: **`internal/cli/` only**, excluding this generated AGENTS.md. Go package name: `cli`. Ownership kind: infrastructure; integration wave: 2.
...
Local proving focus: Every descriptor subprocess mapping, stdin/file/inline equivalence, no TTY, exact stdout/exit, stderr secrets, accepted long task vs failure, bootstrap and key replay parity.
