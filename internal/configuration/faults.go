...
func faultOf(err error) *contract.Fault {
	...
	return internalError("configuration operation failed")
}
...
// diagnostic builds a wire Diagnostic with the given severity.
func diagnostic(severity, path, code, message string) wireDiagnostic {
	return wireDiagnostic{Path: path, Code: code, Message: message, Severity: severity}
}

func errorDiagnostic(path, code, message string) wireDiagnostic {
	return diagnostic("error", path, code, message)
}

func warnDiagnostic(path, code, message string) wireDiagnostic {
	return diagnostic("warning", path, code, message)
}

func infoDiagnostic(path, code, message string) wireDiagnostic {
	return diagnostic("info", path, code, message)
}

// requirement builds a wire Requirement.
func requirement(code, message string, resourceID string) wireRequirement {
	r := wireRequirement{Code: code, Message: message}
	if resourceID != "" {
		r.ResourceID = contract.ID(resourceID)
	}
	return r
}
