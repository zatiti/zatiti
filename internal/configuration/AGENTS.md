...
Own organizations, chiefs, teams, projects, worker definitions, bindings, drafts, plans, revisions and the sole definition compiler.

Write scope: **`internal/configuration/` only**, excluding this generated AGENTS.md. Go package name: `configuration`. Ownership kind: domain; integration wave: 1.
...
Local proving focus: Atomic child/chief creation, ordinary worker no child org, duplicate chief/cycle, stale/concurrent apply, exact replay after head changes, old-policy authorization, scoped move/privacy, archive obligations, export/import/rebinding roundtrip. Rev15: candidate rejects client evidence, mismatched connection/profile/route, evidence digest is server-derived, max qualification charge is enforced, failed/unknown probes never produce a qualified profile, replay returns the same job/result.
