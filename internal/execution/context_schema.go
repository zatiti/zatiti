...
// responsesPrepareSessionBody is the zatiti.responses.action/v1
// prepare_session action schema (ResponsesPrepareSessionParameters): the
// exact fields a hosted worker turn's dispatched prepare_session effect
// must carry -- no model-visible content, no context_artifact. The gap fix
// (execution-dispatch-model-step) is the first caller of this $def;
// ResponsesPrepareSessionParameters was already present in contextSchemaDefs
// (P15 embedded it alongside ResponsesModelStepParameters), only this body
// wrapper and its compose registration are new.
const responsesPrepareSessionBody = `{"$schema":"https://json-schema.org/draft/2020-12/schema","$ref":"#/$defs/ResponsesPrepareSessionParameters"}`

// responsesEvidenceBody is the zatiti.responses.evidence/v1 document schema
// (ResponsesEvidence): the exact normalized adapter observation shape for
// both physical Responses calls (prepare_session and model_step). The gap
// fix's prepare_session confirmation branch (interpretPrepareSessionObservation,
// interpret.go) strictly validates a delivered prepare_session observation
// against this exact frozen shape -- never against modelOutputBody's
// ModelOutput, which is a structurally different, model_step-only document
// -- before trusting its session_handle. ResponsesEvidence itself is newly
// added to contextSchemaDefs above, copied byte-for-byte from
// internal/execution/AGENTS.md's own embedded $defs (verified structurally
// equal via the same technique that produced the rest of this catalog); see
// docs/roadmap.md for how that verification was done.
const responsesEvidenceBody = `{"$schema":"https://json-schema.org/draft/2020-12/schema","$ref":"#/$defs/ResponsesEvidence"}`
...
var (
	...
	composedResponsesPrepareSession json.RawMessage
	composedResponsesEvidence       json.RawMessage
	composeContextSchemaErr         error
)

func composeContextSchemas() error {
	composeContextSchemasOnce.Do(func() {
		var doc map[string]json.RawMessage
		if err := contract.DecodeStrict([]byte(contextSchemaDefs), &doc); err != nil {
			composeContextSchemaErr = fmt.Errorf("execution: embedded adapter $defs catalog is not valid JSON: %w", err)
			return
		}
		defs, ok := doc["$defs"]
		if !ok {
			composeContextSchemaErr = fmt.Errorf("execution: embedded adapter $defs catalog has no $defs object")
			return
		}
		targets := []struct {
			body string
			...
			{responsesPrepareSessionBody, &composedResponsesPrepareSession},
			{responsesEvidenceBody, &composedResponsesEvidence},
		}
		for _, t := range targets {
			composed, err := withAdapterDefs(t.body, defs)
			if err != nil {
				composeContextSchemaErr = err
				return
			}
			*t.dst = composed
		}
	})
	return composeContextSchemaErr
}

// withAdapterDefs returns body with the adapter $defs catalog injected at
// the document root, so every #/$defs/... reference resolves
// document-locally, matching the exact self-contained shape
// contract.ValidateSchema requires.
func withAdapterDefs(body string, defs json.RawMessage) (json.RawMessage, error) {
	var doc map[string]json.RawMessage
	if err := json.Unmarshal([]byte(body), &doc); err != nil {
		return nil, fmt.Errorf("execution: adapter operation schema is not valid JSON: %w", err)
	}
	if _, ok := doc["$defs"]; ok {
		return nil, fmt.Errorf("execution: adapter operation schema already declares $defs: %s", body)
	}
	doc["$defs"] = defs
	out, err := json.Marshal(doc)
	if err != nil {
		return nil, fmt.Errorf("execution: adapter schema composition failed: %w", err)
	}
	return json.RawMessage(out), nil
}

// contextArtifactSchema returns the composed zatiti.context/v1 schema.
func contextArtifactSchema() (json.RawMessage, error) {
	if err := composeContextSchemas(); err != nil {
		return nil, err
	}
	return composedContextArtifact, nil
}

// responsesModelStepSchema returns the composed zatiti.responses.action/v1
// model_step schema.
func responsesModelStepSchema() (json.RawMessage, error) {
	if err := composeContextSchemas(); err != nil {
		return nil, err
	}
...
// responsesPrepareSessionSchema returns the composed
// zatiti.responses.action/v1 prepare_session schema.
func responsesPrepareSessionSchema() (json.RawMessage, error) {
	if err := composeContextSchemas(); err != nil {
		return nil, err
	}
	return composedResponsesPrepareSession, nil
}

// responsesEvidenceSchema returns the composed zatiti.responses.evidence/v1
// schema: the gap fix's prepare_session confirmation branch strictly
// validates a delivered prepare_session observation against this exact
// frozen shape before trusting its session_handle -- never against
// modelOutputSchema, which validates a structurally different document.
func responsesEvidenceSchema() (json.RawMessage, error) {
	if err := composeContextSchemas(); err != nil {
		return nil, err
	}
	return composedResponsesEvidence, nil
}
