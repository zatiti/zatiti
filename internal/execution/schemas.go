...
const (
	...
	schemaInExecutionTurnObservation     = `{"type":"object","additionalProperties":false,"properties":{"turn_id":{"type":"string","format":"uuid"},"step_index":{"type":"integer","minimum":0,"maximum":9223372036854775807},"operation_id":{"type":"string","format":"uuid"},"observation":{"type":"object"}},"required":["turn_id","step_index","operation_id","observation"]}`
	...
	schemaInExecutionContextCommit       = `{"type":"object","additionalProperties":false,"properties":{"plan_id":{"type":"string","format":"uuid"},"expected_version":{"type":"integer","minimum":1,"maximum":9223372036854775807},"generation":{"type":"integer","minimum":1,"maximum":9223372036854775807},"staged_context":{"$ref":"#/$defs/Adapter_ArtifactLocator"}},"required":["plan_id","expected_version","generation","staged_context"]}`
	schemaInExecutionProposalPrepare     = `{"type":"object","additionalProperties":false,"properties":{"turn_id":{"type":"string","format":"uuid"},"step_index":{"type":"integer","minimum":0,"maximum":9223372036854775807},"proposal_id":{"type":"string","maxLength":8192},"expected_version":{"type":"integer","minimum":1,"maximum":9223372036854775807}},"required":["turn_id","step_index","proposal_id","expected_version"]}`
	schemaInExecutionProposalRecord      = `{"type":"object","additionalProperties":false,"properties":{"proposal_id":{"type":"string","maxLength":8192},"expected_version":{"type":"integer","minimum":1,"maximum":9223372036854775807},"command_id":{"type":"string","format":"uuid"},"effect_operation_id":{"type":"string","format":"uuid"},"result_artifact":{"$ref":"#/$defs/ArtifactRef"}},"required":["proposal_id","expected_version"]}`
	schemaInExecutionReport              = `{"type":"object","additionalProperties":false,"properties":{"scope":{"$ref":"#/$defs/Scope"},"attempt_id":{"type":"string","format":"uuid"},"lease_id":{"type":"string","format":"uuid"},"generation":{"type":"integer","minimum":1,"maximum":9223372036854775807},"expected_version":{"type":"integer","minimum":1,"maximum":9223372036854775807},"outputs":{"type":"array","items":{"$ref":"#/$defs/ArtifactRef"},"maxItems":4096},"observations":{"type":"object","description":"Inert JSON data bounded by the enclosing size limit; never executable authority."},"usage":{"$ref":"#/$defs/Usage"}},"required":["scope","attempt_id","lease_id","generation","expected_version","outputs","observations","usage"]}`
	schemaInExecutionVerificationPending = `{"type":"object","additionalProperties":false,"properties":{"limit":{"type":"integer","minimum":1,"maximum":100}},"required":["limit"]}`
	schemaInExecutionVerificationClaim   = `{"type":"object","additionalProperties":false,"properties":{"request_id":{"type":"string","format":"uuid"},"expected_version":{"type":"integer","minimum":1,"maximum":9223372036854775807},"generation":{"type":"integer","minimum":1,"maximum":9223372036854775807}},"required":["request_id","expected_version","generation"]}`

	schemaOutExecutionTurn                = `{"type":"object","additionalProperties":false,"properties":{"resource":{"$ref":"#/$defs/WorkerTurn"}},"required":["resource"]}`
	schemaOutExecutionWorkPending         = `{"type":"object","additionalProperties":false,"properties":{"items":{"type":"array","items":{"$ref":"#/$defs/WorkItem"},"maxItems":100}},"required":["items"]}`
	schemaOutExecutionWorkClaim           = `{"type":"object","additionalProperties":false,"properties":{"item":{"$ref":"#/$defs/WorkItem"},"claim_token":{"type":"string","maxLength":8192},"version":{"type":"integer","minimum":1,"maximum":9223372036854775807}},"required":["item","claim_token","version"]}`
...
var opSchemas = map[string][2]string{
	...
	"_execution.context.commit":       {schemaInExecutionContextCommit, schemaOutExecutionTurn},
	"_execution.proposal.prepare":     {schemaInExecutionProposalPrepare, schemaOutExecutionProposal},
	"_execution.proposal.record":      {schemaInExecutionProposalRecord, schemaOutExecutionTurn},
	"_execution.report":               {schemaInExecutionReport, schemaOutAttempt},
	"_execution.verification.pending": {schemaInExecutionVerificationPending, schemaOutExecutionVerificationPending},
	"_execution.verification.claim":   {schemaInExecutionVerificationClaim, schemaOutExecutionVerificationClaim},
}
