...
Outgoing owner calls: `_identity.authority`, `_configuration.snapshot`, `_policy.check`, `_reviews.ensure`, `_reviews.check`, `_accounting.reserve`, `_accounting.settle`, `_accounting.inspect`, `_connections.resolve`, `_connections.discovery.record`, `_connections.validation.record`, `_tasks.snapshot`, `_memory.select`, `_artifacts.metadata`, `_execution.job.create`, `_execution.job.record`. Each exact input/output schema appears below. Calls retain current Unit/authority; owner allowlists are mandatory.
