...
func budgetUnavailable(format string, args ...any) *contract.Fault {
	return fault(contract.CodeBudgetUnavailable, format, args...)
}
...
// closed as artifact_fault without leaking the underlying message.
func blobFault(err error) *contract.Fault {
	var f *contract.Fault
	if errors.As(err, &f) && f != nil {
		return f
	}
	return artifactFault("responses: a blob store operation failed")
}
...
func secretFault(err error) *contract.Fault {
	...
	return internalError("responses: resolving the credential reference failed")
}
