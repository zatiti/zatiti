...
// PROTOCOL.md records. It deliberately holds no HTTP client, secret store or
// blob store: at this pin no operation can be dispatched, so "zero physical
// calls" is a property of the type, not of control flow.
type Adapter struct {
...
// and claim nothing that upstream does not provide (see loadProfile).
func New(deps contract.AdapterDependencies, raw json.RawMessage) (contract.Adapter, error) {
	if deps.Clock == nil {
		return nil, invalidInput("serenity adapter requires a clock dependency")
	}
	...
	return &Adapter{profile: profile, clock: deps.Clock, schema: contractDoc}, nil
}
...
// therefore sends nothing and reports the outcome as still unknown with a
// non-authoritative lookup. It never repeats the original call: an upstream
// replay can insert a second fact or resurrect a retracted one.
func (a *Adapter) Reconcile(_ context.Context, dispatch contract.Dispatch) (contract.Observation, error) {
	act, brain, err := a.admit(dispatch)
	if err != nil {
		return contract.Observation{}, err
	}
	now := a.clock.Now().UTC()
	built, err := a.buildUnknownEvidence(dispatch, act, brain, now)
	if err != nil {
		return contract.Observation{}, err
	}
...
func (a *Adapter) admit(dispatch contract.Dispatch) (*action, wireBrainMapping, error) {
	if dispatch.Adapter != adapterName {
		return nil, wireBrainMapping{}, invalidInput("dispatch adapter %q does not match %q", dispatch.Adapter, adapterName)
	}
	if dispatch.CredentialRef == "" {
		return nil, wireBrainMapping{}, invalidInput("dispatch credential_ref is empty")
	}
	act, err := decodeAction(dispatch.Action)
	if err != nil {
		return nil, wireBrainMapping{}, err
	}
	brain, ok := a.profile.Brains[act.BrainID]
	if !ok {
		return nil, wireBrainMapping{}, permissionDenied("brain %s is not in the profile's brain_mappings; the adapter cannot widen a query", act.BrainID)
	}
	if act.WriterOwner != "" && act.WriterOwner != brain.WriterOwner {
		return nil, wireBrainMapping{}, permissionDenied("writer_owner does not own brain %s; each brain has exactly one canonical writer", act.BrainID)
	}
	if err := a.withinBounds(act); err != nil {
		return nil, wireBrainMapping{}, err
	}
	return act, brain, nil
}
