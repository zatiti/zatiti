...
Expose `New(contract.AdapterDependencies,json.RawMessage) (contract.Adapter,error)`. The profile/action/evidence schemas below are the local frozen seam. Upstream translation must use the qualified pinned public API; one Invoke means one accounted physical call. No arbitrary model-supplied endpoint/account.
