...
const (
	eventScheduleActivated       = "scheduling.schedule.activated"
	eventScheduleArchived        = "scheduling.schedule.archived"
	eventSchedulePaused          = "scheduling.schedule.paused"
	eventScheduleResumed         = "scheduling.schedule.resumed"
	eventResponsibilityActivated = "scheduling.responsibility.activated"
	eventResponsibilityArchived  = "scheduling.responsibility.archived"
...
func emitTransition(ctx context.Context, unit contract.Unit, kind string, resourceID contract.ID, version contract.Version) error {
	...
	if err != nil {
		return fmt.Errorf("scheduling: emit %s: %w", kind, err)
	}
	return nil
}
...
func listResult(items any, next *string) (contract.Payload, error) {
	...
	if err != nil {
		return contract.Payload{}, fmt.Errorf("scheduling: encode list page: %w", err)
	}
...
func (s *Service) cursorKeyLocked() ([]byte, error) {
	if len(s.cursorKey) == 0 {
		...
		if _, err := rand.Read(key); err != nil {
			return nil, fmt.Errorf("scheduling: mint cursor key: %w", err)
		}
		s.cursorKey = key
	}
...
func (s *Service) mintCursor(op string, unitScope contract.Scope, filter any, lastCreated time.Time, lastID contract.ID, now time.Time) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("scheduling: bind cursor filter: %w", err)
	}
...
func (s *Service) readCursor(op string, unitScope contract.Scope, filter any, raw string) (createdAt time.Time, lastID contract.ID, err error) {
	...
	filterJSON, err := json.Marshal(filter)
	if err != nil {
		return time.Time{}, "", fmt.Errorf("scheduling: bind cursor filter: %w", err)
	}
