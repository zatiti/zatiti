...
### P00-012 (source section P00; primary owner registry)

Registry.Lookup(id, 0) resolves the highest registered version for that operation id. Registration accepts either a bare $defs-relative schema or a fully self-contained document, and Lookup always returns a self-contained document pruned to reachable $defs. These rules were implemented and tested against application's assumptions before this revision; this is their authored contract record.
