# Implementation assignment: `internal/server`
...
Own private socket HTTP/JSON and explicit remote-desktop mutual TLS transport.

Write scope: **`internal/server/` only**, excluding this generated AGENTS.md. Go package name: `server`. Ownership kind: infrastructure; integration wave: 2.

Allowed production imports from this repository: `github.com/zatiti/zatiti/internal/contract`, `github.com/zatiti/zatiti/internal/application`. Tests may use interfaces/fakes defined locally and, once available, storage-backed temporary fixtures; integration owns cross-package tests. No sibling raw SQL. No root dependency edits except the integration exception stated in its own brief.
...
Local proving focus: Unix permissions, profile-name spoof, token redaction, TLS principal/revocation, remote bootstrap denial, malformed/oversized body, disconnect after commit and key recovery.
