...
const (
	eventPrincipalCreated = "identity.principal.created"
	eventPrincipalUpdated = "identity.principal.updated"
	eventPrincipalRevoked = "identity.principal.revoked"
	eventGrantCreated     = "identity.grant.created"
	eventGrantUpdated     = "identity.grant.updated"
	eventGrantRevoked     = "identity.grant.revoked"
	eventCredProvisioned  = "identity.credential.provisioned"
	eventCredRevoked      = "identity.credential.revoked"
	eventRestriction      = "identity.restriction.applied"
	eventPromotion        = "identity.promotion.activated"
	eventBootstrap        = "identity.bootstrap.completed"
)
...
func emitTransition(ctx context.Context, unit contract.Unit, kind string, resourceID contract.ID, version contract.Version) error {
	...
	if err != nil {
		return fmt.Errorf("identity: emit %s: %w", kind, err)
	}
	return nil
}
...
func listResult(items any, next *string) (contract.Payload, error) {
	...
	if err != nil {
		return contract.Payload{}, fmt.Errorf("identity: encode list page: %w", err)
	}
...
func (s *Service) cursorKeyLocked() ([]byte, error) {
	if len(s.cursorKey) == 0 {
		...
		if _, err := rand.Read(key); err != nil {
			return nil, fmt.Errorf("identity: mint cursor key: %w", err)
		}
		s.cursorKey = key
	}
...
func (s *Service) mintCursor(op string, unitScope contract.Scope, filter any, lastCreated time.Time, lastID contract.ID, now time.Time) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("identity: bind cursor filter: %w", err)
	}
...
func (s *Service) readCursor(op string, unitScope contract.Scope, filter any, raw string) (createdAt time.Time, lastID contract.ID, err error) {
	...
	filterJSON, err := json.Marshal(filter)
	if err != nil {
		return time.Time{}, "", fmt.Errorf("identity: bind cursor filter: %w", err)
	}
