...
const (
	eventReviewRequested = "reviews.review.requested"
	eventReviewExpired   = "reviews.review.expired"
	eventReviewDecided   = "reviews.review.decided"
	eventReviewDelegated = "reviews.review.delegated"
)
...
func emitTransition(ctx context.Context, unit contract.Unit, kind string, resourceID contract.ID, version contract.Version) error {
	...
	if err != nil {
		return fmt.Errorf("reviews: emit %s: %w", kind, err)
	}
	return nil
}

// emitReviewEvent appends one state-correlated event carrying an inert data
// payload with the transition details.
func emitReviewEvent(ctx context.Context, unit contract.Unit, kind string, reviewID contract.ID, version int64, data any) error {
	raw, err := json.Marshal(data)
	if err != nil {
		return fmt.Errorf("reviews: encode %s data: %w", kind, err)
	}
	err = unit.Emit(ctx, contract.Event{
		Kind:            kind,
		ResourceID:      reviewID,
...
func listResult(items any, next *string) (contract.Payload, error) {
	...
	if err != nil {
		return contract.Payload{}, fmt.Errorf("reviews: encode list page: %w", err)
	}
...
func (s *Service) cursorKeyLocked() ([]byte, error) {
	if len(s.cursorKey) == 0 {
		...
		if _, err := rand.Read(key); err != nil {
			return nil, fmt.Errorf("reviews: mint cursor key: %w", err)
		}
		s.cursorKey = key
	}
...
func (s *Service) mintCursor(op string, unitScope contract.Scope, filter any, lastCreated time.Time, lastID contract.ID, now time.Time) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("reviews: bind cursor filter: %w", err)
	}
...
func (s *Service) readCursor(op string, unitScope contract.Scope, filter any, raw string) (createdAt time.Time, lastID contract.ID, err error) {
	...
	filterJSON, err := json.Marshal(filter)
	if err != nil {
		return time.Time{}, "", fmt.Errorf("reviews: bind cursor filter: %w", err)
	}
