...
// Service assembly. Evidence constructs as New(deps), registers every owned
// operation through the local bind (the frozen registry.Bind contract,
// mirrored locally because evidence may only import the shared contract),
// and dispatches strictly. Evidence makes no outgoing peer calls: it is a
// pure recorder and reader over its own commands and storage's event feed,
// so only a clock and an identity source are required.

// ownerName is the domain owner identity used in descriptors and events.
const ownerName = "evidence"

// Operation identifiers served by this module.
const (
	opCommandBegin  = "_evidence.command.begin"
	opCommandFinish = "_evidence.command.finish"
	opSnapshot      = "_evidence.snapshot"
	opCommandGet    = "command.get"
	opEventGet      = "event.get"
	opEventList     = "event.list"
)
...
var opMetas = []opMeta{
	{id: opCommandBegin, visibility: contract.VisibilityInternal, mode: contract.ModeMutation,
		callers: []string{"application"}},
	{id: opCommandFinish, visibility: contract.VisibilityInternal, mode: contract.ModeMutation,
		callers: []string{"application"}},
...
// event reads and receipts projected from actual evidence.
type Service struct {
...
// New constructs the evidence owner. It never queries peers, touches
// storage or starts goroutines; all runtime coupling arrives through deps.
func New(deps contract.Dependencies) (*Service, error) {
	if deps.Clock == nil {
		return nil, &contract.Fault{Code: contract.CodeInternalError, Message: "evidence requires a clock"}
	}
	if deps.IDs == nil {
		return nil, &contract.Fault{Code: contract.CodeInternalError, Message: "evidence requires an identity source"}
	}
	s := &Service{
		deps:     deps,
...
func (s *Service) assemble() error {
	...
	for _, m := range opMetas {
		...
		if !ok {
			return fmt.Errorf("evidence: operation %s has no wire schemas", m.id)
		}
		inputSchema, err := mergeSchema(schemas.Input)
		if err != nil {
			return fmt.Errorf("evidence: operation %s input schema: %w", m.id, err)
		}
		outputSchema, err := mergeSchema(schemas.Output)
		if err != nil {
			return fmt.Errorf("evidence: operation %s output schema: %w", m.id, err)
		}
		...
		if err != nil {
			return fmt.Errorf("evidence: bind %s: %w", m.id, err)
		}
		s.catalog[m.id] = d
		s.handlers[m.id] = h
...
func (s *Service) bindHandler(d contract.Descriptor) (contract.Handler, error) {
	switch d.ID {
	case opCommandBegin:
		return bind(d, s.handleCommandBegin)
	case opCommandFinish:
		return bind(d, s.handleCommandFinish)
	case opSnapshot:
		return bind(d, s.handleSnapshot)
	case opCommandGet:
		return bind(d, s.handleCommandGet)
	case opEventGet:
		return bind(d, s.handleEventGet)
	case opEventList:
		return bind(d, s.handleEventList)
	default:
...
// shape checks mirror the registry's; internal operations do not require a
...
// closed on a broken schema instead of failing on the wire. The document
// decodes into a dynamic (map[string]any / []any) tree rather than
// map[string]json.RawMessage specifically so the recursive walk below can
// match on that same dynamic shape at every depth, not just the top level.
func checkSchemaDocument(doc json.RawMessage) error {
	...
	if !ok {
		return fmt.Errorf("schema $defs is not a JSON object")
	}
	return resolveRefs(obj, defs)
}

// resolveRefs walks one decoded schema node checking every local $ref.
func resolveRefs(node any, defs map[string]any) error {
	switch v := node.(type) {
	case map[string]any:
