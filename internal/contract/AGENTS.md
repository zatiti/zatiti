...
Own shared Go types, strict wire validation, canonicalization, schema-derived primitives and stable faults.

Write scope: **`internal/contract/` only**, excluding this generated AGENTS.md. Go package name: `contract`. Ownership kind: foundation; integration wave: 0.
...
Implement the exact declarations in the shared contract. No domain services, persistence, SDKs, global configuration or controller side effects. Provide Canonicalize([]byte) ([]byte,error), Hash([]byte) Digest, DecodeStrict([]byte,any) error, ValidateSchema(json.RawMessage,json.RawMessage) error, NewID() ID, and CLIExit(*Fault) int / HTTPStatus(*Fault) int. Hash hashes exact bytes; callers canonicalize JSON first. Validate JSON Schema 2020-12, local $defs/$ref, formats, oneOf/anyOf, strict objects, integer bounds and collection limits; never fetch remote schema references. Registered tool/skill JSON schemas are untrusted bounded data and cannot execute callbacks. Public identifiers/enums and generated DTO names are fixed; behavior-private helpers remain local. Schema validation and decoding must agree on duplicate keys and integer precision. Keep descriptor data secret-free.

Local proving focus: Canonical JSON golden vectors with duplicate keys, integer overflow, semantic array order/set sorting, exact skill bytes; typed decode errors; all error/exit/HTTP mappings; UUID and digest validation.
