...
internal('metadata','artifacts',obj(scope=ref('Scope'),artifacts=arr(obj(id=ID,**{'digest?':DIG}))),obj(artifacts=arr(ref('Artifact'))),'Validate scope, availability and classification of pinned artifacts, by id alone or id-plus-digest, before disclosure or acceptance; a caller with only an id (evidence and acceptance references never carry one) resolves by id, and one that also supplies a digest additionally fences the match against it.',['execution','tasks','effects','memory','messaging','skills','installation'],mode='query')
