...
func TestPolicyRejectsMutations(t *testing.T) {
	...
	cases := []mutation{
		...
		{"gate removed from native matrix needs", rel, "      - qualification\n", "", false, "native-evidence"},
		...
		{"native qualification matrix gate dropped", rel, "      - qualification_matrix\n    runs-on: ubuntu-24.04\n", "    runs-on: ubuntu-24.04\n", false, "release-gates"},
		{"native Intel report omitted", rel, "name: release-evidence-qualification-macos-15-intel\n", "name: release-evidence-qualification-macos-15-missing\n", false, "native-evidence"},
		{"native report comparison omitted", rel, "cigate qualmatrix \\\n", "cigate environ \\\n", false, "native-evidence"},
		{"native Mac candidate disables Keychain", rel, "          CGO_ENABLED: \"1\"\n", "          CGO_ENABLED: \"0\"\n", false, "native-keychain"},
		{"native Mac candidate drops binary cgo check", rel, "          go version -m \"$RUNNER_TEMP/candidate/zatiti\" | grep -Eq '^[[:space:]]*build[[:space:]]+CGO_ENABLED=1$'\n", "", false, "native-keychain"},
		{"native Mac candidate drops Keychain symbol check", rel, "          nm \"$RUNNER_TEMP/candidate/zatiti\" | grep -F '_SecItemAdd' >/dev/null\n", "", false, "native-keychain"},
		...
		{"qualification evidence enumeration step removed", rel, "go run ./.github/workflows/cigate qualevidence \\\n", "echo removed \\\n", false, "release-gates"},
		{"native release case omitted", rel, ",QUALIFICATION.macos_serenity_hard_gate", "", false, "release-cases"},
		{"native platform evidence omitted", rel, `-platform "darwin/$QUALIFICATION_ARCH"`, `-platform "darwin/amd64"`, false, "release-cases"},
	}
