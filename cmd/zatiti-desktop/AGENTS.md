# Implementation assignment: `cmd/zatiti-desktop`
...
Assemble the separately packaged native desktop client.

Write scope: **`cmd/zatiti-desktop/` only**, excluding this generated AGENTS.md. Go package name: `main`. Ownership kind: entrypoint; integration wave: 3.

Allowed production imports from this repository: `github.com/zatiti/zatiti/internal/contract`, `github.com/zatiti/zatiti/internal/platform`, `github.com/zatiti/zatiti/internal/client`, `github.com/zatiti/zatiti/internal/desktop`. Tests may use interfaces/fakes defined locally and, once available, storage-backed temporary fixtures; integration owns cross-package tests. No sibling raw SQL. No root dependency edits except the integration exception stated in its own brief.

## Implementation decisions and acceptance focus

main delegates testable run; load explicit protected client config/profile, construct shared client and desktop. Setup may request local controller service installation/start through explicit OS helper but closing desktop does not own controller shutdown. Remote endpoint only explicit TLS configured. No DB, domain modules, scheduler or model provider directly in desktop process. OS-specific lifecycle integration is packaged/qualified by distribution owner. Surface startup prerequisites without raw secret/path disclosure in conversations.

Local proving focus: Real GUI launch on macOS/Linux, existing controller reconnect, close without server stop, secure credential setup and remote TLS.
...
Real GUI launch on macOS/Linux, existing controller reconnect, close without server stop, secure credential setup and remote TLS.
