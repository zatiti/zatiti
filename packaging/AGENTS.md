...
Own release manifests, service launchers, secure helper and Serenity distribution lifecycle.

Write scope: **`packaging/` only**, excluding this generated AGENTS.md. Go package name: `distribution`. Ownership kind: infrastructure; integration wave: 4.
...
Local proving focus: Clean native arm64 and amd64 macOS install/first-chat/launch/upgrade/restart/uninstall preserve state, secure permissions, controller survives desktop close, Serenity lifecycle and license notice audit; Linux requires later separate qualification. Rev9: fake package payload/script, cyclic or mismatched binding, wrong signer/arch, post-install tamper, interrupted Apply/fence, helper bundle identity and no state loss.
