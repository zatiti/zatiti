...
This is a non-Go ownership root: commit platform service templates/scripts/manifests and qualification instructions. macOS LaunchAgent and Linux user systemd unit keep controller alive independently of desktop. Package binaries plus pinned Serenity runtime/read facade as qualified; one writer owner per brain and one controller state lock. Artifact manifest includes version/checksum/licenses/SBOM, supported protocol/profiles and required native Fyne dependencies. Secret/key provisioning uses trusted local OS helper, never flags/chat/environment inherited by worker. Install/uninstall preserve state unless explicit operator removal requested; no invented working install command before built artifacts tested. Avoid signing/notarization/publishing claims without actual evidence. Root release CI is separate workflow owner. Backup/restoration key prerequisites documented without private paths or real secrets.

Local proving focus: Clean macOS/Linux install/launch/upgrade/restart/uninstall preserve state, secure permissions, controller survives desktop close, Serenity lifecycle and license notice audit.
...
Clean macOS/Linux install/launch/upgrade/restart/uninstall preserve state, secure permissions, controller survives desktop close, Serenity lifecycle and license notice audit.
