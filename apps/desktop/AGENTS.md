...
Process lifecycle: main() delegates to a testable run; load explicit protected client configuration/profile. Setup may request local controller service installation/start through the explicit packaged OS helper, but closing the client never stops the controller, and this is disclosed during setup. Remote endpoint only when explicitly configured with TLS. Surface startup prerequisites without raw secret or filesystem path disclosure in conversations. OS-specific lifecycle integration and bundle assembly are packaged/qualified by the distribution owner. Revision 6: installed Mac startup reads strict protected desktop.json, obtains the existing owner header from the named login-Keychain item, and authenticates installation.status before opening chat; distinguish repair states and reconcile ambiguous init. Revision 9: Mac Runner (not Dart) verifies and directly launches the signed controller-distribution helper at the fixed installed path $HOME/Library/Application Support/zatiti-dist/current/bin/zatiti-credential-helper for the strict nonsecret zatiti.gui-credential-capture/v1 request; Dart receives only bounded redacted result enum, then queries authoritative connection status. No provider key, receipt, opaque ref, owner header, path or master selector in method-channel messages. Remote/dev/headless profiles refuse this installed-only capability; the desktop bundle continues to carry no helper. Revision 11: after local owner/chief initialization, guide Sign in to Serenity, show existing project selection in the hosted consent page, wait on authoritative challenge/connection status, display the verified account/project and chosen read/write scopes, then request model-provider setup and activate the chief. Keep composer gated until actual Serenity and provider readiness; offer reconnect without silently switching memory. Revision 15: provide a model-profile setup wizard that selects a saved provider connection, accepts model, explicit pricing/resource limits and classification, asks for the qualification spend bound, and submits only ResponsesProfileDraft. Poll the same job by identity through unknown/lost acknowledgements; never replace its submission key or replay a possibly sent probe. On trusted completion, show observed route/model/cost limitations, then create the execution profile and use the existing draft → plan → user-reviewed apply path. Never accept, edit or synthesize capability_evidence.

Local proving focus: Dart unit/widget tests against a controlled fake server speaking the real envelope: strict envelope rejection and int64 fidelity, disconnect before/after commit with original-key replay, unknown acknowledgment queried before retry, expired cursor snapshot recovery, pending versus acknowledged approval/pause/creation, offline stale/unsent labels, quiet coordination, secure-storage-unavailable prerequisite, no credential in logs, catalog digest drift check. Real first-chief and two-child-chief journeys, breadcrumbs/filter, exact approval and close-while-controller-runs are driven against a real controller by tests/qualification. Rev9: native channel schema rejection, caller/signature gating, cancellation, keyed crash reconciliation and secret-absence probes; native signed UI test is a release gate. Rev11: keyboard/VoiceOver/200-percent sign-in, cancel/retry/reconnect, wrong project and no-token-in-Dart checks. Rev15: keyboard-accessible provider/model setup, profile draft without evidence, explicit probe-cost review, job reconciliation after lost acknowledgement, failed/unknown qualification cannot be selected, qualified result requires reviewed apply.
...
Admit one bounded, separately authorized MCP tools/list catalog read for the selected connection. It installs no authority; each discovered tool needs an explicit binding and profile allowlist entry.
...
List the locally recorded discovered tool catalog under current authorization; this operation makes no live provider call. Filters are structured exact-match fields (AND semantics); unsupported fields for this resource refuse invalid_input. Never interpolate filter strings as SQL.
