# Implementation assignment: `apps/desktop`

Generated specification revision 4; source digest `c1ebefef4c17ed32e6f474a3be1158de0c0a2942207e8d32b1d856ce0101c082`. This file is committed implementation context. Do not independently edit it. Everything required from the product specification and adjacent interfaces is embedded below; no RFC copy is required.

## Mission and scope

Own the Flutter chat-first human workspace: a direct wire client of the controller with acknowledged controller-derived views.

Write scope: **`apps/desktop/` only**, excluding this generated AGENTS.md. Go package name: `not a Go package`. Ownership kind: client; integration wave: 3.

Repository boundary: consumes docs/implementation/operations.json and the versioned wire envelope served by internal/server; no Go imports. Allowed Go imports do not apply: this root is outside the Go module, imports no Go package and is imported by none. It has no database, state-directory or sibling-package access; its only product interface is the public operation catalog over the wire contract below. Tests use local fakes that speak the real envelope; integration and qualification own cross-process proving. No root dependency edits.

## Implementation decisions and acceptance focus

This root is a Flutter application written in Dart (ADR 002). It is not a Go package, is not imported by the Go module and replaces the retired Go roots internal/desktop and cmd/zatiti-desktop. Own pubspec.yaml/pubspec.lock, lib/, test/, integration_test/ and the macos/ and linux/ runner directories inside this root. Pin exact Flutter SDK, Dart SDK and plugin versions in pubspec.lock and report them with licenses to the integration owner for the dependency lock report; no unpinned or git-head plugin. No Go code, no sidecar process, no bundled controller library and no WebView wrapper presented as the Flutter implementation.
Transport: speak directly to internal/server. Locally, HTTP/1.1 over the private Unix socket; for an explicitly configured remote desktop only, the mutual-TLS listener with a provisioned client certificate. POST /v1/operations/{operation_id} with the versioned zatiti.request/v1 envelope and strict zatiti.result/v1 decoding: reject unknown fields, duplicate keys and malformed envelopes, bound body reads, preserve exact int64 values (never route 64-bit integers through a double). Completed 200 and accepted 202 are distinct; domain failures arrive inside the envelope with the shared HTTP mapping. Unix-socket HTTP support in the pinned Dart SDK on macOS and Linux is qualification work, not assumed. Windows is not a v1 target; its transport choice (AF_UNIX in Dart versus a mutual-TLS loopback listener) stays open until qualified.
Operation catalog: generate Dart request/response bindings at build time from docs/implementation/operations.json as a read-only input. Do not commit a hand-edited or forked catalog copy; a build/test check compares the catalog digest the bindings were generated from with the committed catalog. At connect, confirm required operations and versions through capabilities.list and surface a missing or mismatched operation as a named unsupported state, never a guessed call. Only public operations exist for this client; it cannot supply Actor, Unit, scope authority or an internal operation.
Credentials: client credential, client TLS key material and offline drafts live in operating-system secure storage through a pinned Flutter plugin (macOS Keychain, Linux Secret Service). No plaintext fallback when secure storage is unavailable: report a named prerequisite. The credential profile is startup configuration and never an operation argument, conversation text, log line or crash report. The credential travels only in the Authorization header. Never place tokens in UI model context.
No business logic and no database access: no SQLite, no state-directory reads, no policy/authority/budget evaluation, no scheduler, no model provider and no adapter in this process. Every displayed state derives from controller snapshots and events. Reimplement in Dart the exact retry semantics of the embedded internal/client brief: automatic retry only for connection establishment before any request bytes are written, or an explicit identical resubmission with the ORIGINAL submission key; never replace a mutation key after a timeout; resolve an unknown acknowledgment with command.get before any retry; cursor_expired with snapshot_required refreshes a snapshot before replay and never fills a gap silently. Cancelling a local wait is not cancellation of an accepted command.
Acknowledgment rule: approval, pause/stop, delivery and creation are shown as done only after the controller acknowledges the command. Until then the view shows pending; on an unknown acknowledgment it shows unknown and queries. Cards distinguish proposed / awaiting decision / committed created, and actions show controller disposition, not model narration.
Workspace (design of record: internal/desktop/design/, a committed HTML/CSS/JS study that stays at that path until a follow-up revision relocates it; read it, do not move, edit or delete it from this root, and do not treat its browser rendering as evidence for the Flutter build): left sidebar All chats and optional organization selector, main conversation list/selected conversation, composer, optional details panel. Initial onboarding runs necessary bootstrap/connection/limits prerequisites then opens the pinned personal-chief chat with a ready composer; returning launch restores the selected conversation. New menu worker/org/group; org optional parent defaults to current context. Chat and direct controls stage exactly the same compiler objects. Org filter includes descendants, worker label shows muted org, clickable full ancestry breadcrumb, disambiguating search, not color-only membership. User can speak directly to any worker. Group chats create no org/grants/memory permissions. Durable task/responsibility/files/reviews/memory remain discoverable outside scroll. Needs you shows exact current human decisions and links evidence. Quiet internal reasoning does not reorder/unread/notify. Offline display labels cached/stale and unsent drafts. Capability-specific autonomy cards show exact evidence/rule/authority change, not a global trust score. Pause/stop controls report acknowledgment and unknown effects. Memory UI shows scope/sources/freshness and active recall removal vs historical erasure. Keyboard and screen-reader access are requirements to establish on each platform, not claims.
Process lifecycle: main() delegates to a testable run; load explicit protected client configuration/profile. Setup may request local controller service installation/start through the explicit packaged OS helper, but closing the client never stops the controller, and this is disclosed during setup. Remote endpoint only when explicitly configured with TLS. Surface startup prerequisites without raw secret or filesystem path disclosure in conversations. OS-specific lifecycle integration and bundle assembly are packaged/qualified by the distribution owner.

Local proving focus: Dart unit/widget tests against a controlled fake server speaking the real envelope: strict envelope rejection and int64 fidelity, disconnect before/after commit with original-key replay, unknown acknowledgment queried before retry, expired cursor snapshot recovery, pending versus acknowledged approval/pause/creation, offline stale/unsent labels, quiet coordination, secure-storage-unavailable prerequisite, no credential in logs, catalog digest drift check. Real first-chief and two-child-chief journeys, breadcrumbs/filter, exact approval and close-while-controller-runs are driven against a real controller by tests/qualification.

## Incoming and outgoing boundaries

Incoming callers: none; this root is a separate client process and exposes no API to the repository.

Outgoing owner calls: none internal; only public operations through the internal/server transport as the authenticated human principal. Each exact input/output schema appears below. Calls retain current Unit/authority; owner allowlists are mandatory.

### Reference package briefs (wire behavior to match; not imports)
...
### `connection.discover` v1 — connections / public / mutation / external_read

CLI `zatiti connection discover`; MCP `zatiti_connection_discover`. Submission key: required.

Admit a bounded, separately authorized tool-catalog read (adapter kind list_tools; provider mcp only, capability_unsupported for every other provider). Records the observed tool names, pinned input schemas, digests and server annotations through _connections.discovery.record. Discovery installs no authority: a discovered tool is callable only after an explicit binding names it and the profile allowlist permits it.
...
### `connection.tools` v1 — connections / public / query / local

CLI `zatiti connection tools`; MCP `zatiti_connection_tools`. Submission key: not required at this internal/query/bootstrap boundary.

List the connection's recorded discovered tools with pinned schema digests under current authorization; never a live server call. Filters are structured exact-match fields (AND semantics); unsupported fields for this resource refuse invalid_input. Never interpolate filter strings as SQL.
...
Output data schema:
```json
{"type":"object","additionalProperties":false,"properties":{"items":{"type":"array","items":{"$ref":"#/$defs/MCPDiscoveredTool"},"maxItems":500}},"required":["items"]}
```
