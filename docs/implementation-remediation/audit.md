...
[PR #2 CI run 35473210732](https://github.com/zatiti/zatiti/actions/runs/35473210732), at documentation commit `f03a9bfdf5d494a78cc0f5a99b0aa52fd09a039d`, passed specification drift, workflow validation, static checks and both Flutter jobs. Both Go jobs failed in `internal/platform`: `TestListenPrivateRefusesSymlinkedRunDirectory` expected `invalid_input` but received nil on Linux and macOS; macOS additionally failed `TestBlobTamperedObjectFailsPublishOverExisting`, expecting `artifact_fault` but receiving nil. The production source is unchanged from the audited baseline. These hosted results qualify the earlier local passing result; they are not evidence of a healthy cross-platform baseline. The root cause is unverified.

P30 owns reproduction and correction within `internal/platform`, preserving the security assertions rather than skipping or weakening tests. P48 owns retention of Linux/macOS regression evidence and release gating. These checks are additional platform work, independent of the absent intent-to-work orchestration.
