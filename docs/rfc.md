...
**Current Mac direction:** the [frozen implementation contract](implementation/contracts.md), revision 11, selects hosted Serenity OAuth and the user's existing personal brain for Mac. Where this RFC's earlier process-packaging language conflicts with that contract, revision 11 is current. It preserves the full memory, cost, recovery, and qualification requirements; it does not claim the hosted adapter or Mac release is complete.
...
The first release includes the chat-first desktop application, CLI and local stdio MCP, a complete shared operation catalog, hierarchical organization authoring, personal and organization chiefs, Serenity-backed memory, versioned skills, connection setup, durable execution, scoped policy, earned autonomy, approvals, evidence, and recovery. Mac setup selects the user's existing hosted Serenity personal brain through browser OAuth; it does not create a duplicate personal brain or bundle a Serenity runtime. The full hosted memory and recovery capabilities remain release gates. It includes one qualified hosted model-provider adapter, the cooperative external-worker protocol, bounded HTTP reads, repository artifact workflows, and one qualified GitHub adapter for explicitly authorized repository actions. Provider/model selections are installation inputs, not hardcoded personal accounts or model aliases.

Initial distribution targets macOS for Intel and Apple Silicon. Linux and Windows desktop follow as qualified builds; iOS, Android, and web are near-term Flutter targets after the Mac path. No advertised platform or agent-client compatibility is established solely by code compiling.

### 2.4 Exclusions

The Mac install-to-first-chat milestone does not include hosted multitenancy for Zatiti, remote MCP over HTTP, high availability, multiple active controllers, automatic controller migration, cloud fleet provisioning, a marketplace, or a universal guarantee of exactly-once effects. Linux/Windows desktop and iOS/Android/web clients follow as separately qualified Flutter targets. Social publishing, external chat ingestion, arbitrary third-party server execution, and additional provider adapters can follow the same contracts later; they do not gate the Mac milestone.
...
Ship a `zatiti` Go controller/CLI binary with CLI commands, `serve`, and `mcp serve`, plus a desktop client and a pinned public Serenity integration. In hosted Mac mode, the controller connects to Serenity's hosted Streamable HTTP MCP service using an OAuth grant bound to one verified brain; no local Serenity service is installed. One always-on controller owns scheduling, admission, persistence, credential access, and recovery. It may run on the user's computer or an operator-controlled server; ongoing work requires that host to remain available. Closing the desktop client does not stop the controller. Optional local execution workers connect to that same owner; moving work between controllers is not a v1 feature.
...
The controller publishes a versioned API contract; a pinned Mint generation step may turn that contract into the MCP server shipped with Zatiti. Mint is a build-time adapter, not a second source of domain behavior. Serenity has a separate canonical memory store and one writer owner per brain. Hosted mode uses public OAuth and MCP endpoints; the adapter must verify the grant's account, selected brain, and scopes before dispatch. Self-hosted Serenity packaging may be added as a separate mode later.
...
For Mac, hosted Serenity's public Streamable HTTP MCP endpoint is the primary transport; the local exported Go read facade is not a packaging dependency. Canonical writes go to exactly one writer owner per brain through the supported protocol. Its Recall path may invoke models and record spend: "read" does not imply no disclosure or no charge. Composition, embedding, extraction, and curation must obey Zatiti's provider disclosure rules and reservations; until the adapter can enforce a required bound, that execution mode is unavailable. Serenity's remembered judgments and plan checks inform work but cannot override Zatiti policy or constitute task acceptance on their own. The hosted deployment still needs public grant identity, command status, revision/export, cost, and disclosure evidence before this adapter can pass the full release gate.
