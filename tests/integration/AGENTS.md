# Implementation assignment: `tests/integration`
...
Own real cross-package fixtures, atomicity, transport parity and end-to-end journeys.

Write scope: **`tests/integration/` only**, excluding this generated AGENTS.md. Go package name: `integration_test`. Ownership kind: verification; integration wave: 4.

Allowed production imports from this repository: `github.com/zatiti/zatiti/internal/contract`, `github.com/zatiti/zatiti/internal/platform`, `github.com/zatiti/zatiti/internal/storage`, `github.com/zatiti/zatiti/internal/identity`, `github.com/zatiti/zatiti/internal/configuration`, `github.com/zatiti/zatiti/internal/skills`, `github.com/zatiti/zatiti/internal/connections`, `github.com/zatiti/zatiti/internal/policy`, `github.com/zatiti/zatiti/internal/reviews`, `github.com/zatiti/zatiti/internal/accounting`, `github.com/zatiti/zatiti/internal/tasks`, `github.com/zatiti/zatiti/internal/scheduling`, `github.com/zatiti/zatiti/internal/messaging`, `github.com/zatiti/zatiti/internal/execution`, `github.com/zatiti/zatiti/internal/effects`, `github.com/zatiti/zatiti/internal/memory`, `github.com/zatiti/zatiti/internal/artifacts`, `github.com/zatiti/zatiti/internal/evidence`, `github.com/zatiti/zatiti/internal/installation`, `github.com/zatiti/zatiti/internal/registry`, `github.com/zatiti/zatiti/internal/application`, `github.com/zatiti/zatiti/internal/controller`, `github.com/zatiti/zatiti/internal/server`, `github.com/zatiti/zatiti/internal/client`, `github.com/zatiti/zatiti/internal/cli`, `github.com/zatiti/zatiti/internal/mcp`, `github.com/zatiti/zatiti/internal/adapters/responses`, `github.com/zatiti/zatiti/internal/adapters/github`, `github.com/zatiti/zatiti/internal/adapters/httpread`, `github.com/zatiti/zatiti/internal/adapters/serenity`, `github.com/zatiti/zatiti/internal/adapters/mcpclient`. Tests may use interfaces/fakes defined locally and, once available, storage-backed temporary fixtures; integration owns cross-package tests. No sibling raw SQL. No root dependency edits except the integration exception stated in its own brief.

## Implementation decisions and acceptance focus

Implement package integration_test. Sole integration owner also has separately serialized root-file assignment for go.mod/go.sum, root build configuration and qualified dependency lock. Never dispatch that root assignment concurrently with child scopes. Establish go module/toolchain and exact library pins before dependent agents start; generate baseline contract package first. Dependency plan is a pinned lock report docs/implementation/dependencies.lock.json with module/version/sum/license, external service commit/protocol, qualified features/gaps and named test commands; this file is an explicit integration-owner exception to test write scope. Do not edit frozen prompts/contracts to make tests pass. Construct real SQLite controllers with controlled provider servers, binding installation.WithDatabaseBackup to a backup-only wrapper over the real storage Database exactly as cmd/zatiti does so backup/restore cases restore the actual bytes, fake clock and secure temporary stores; use actual CLI subprocesses and MCP client, not direct handler-only parity. Compare identical principal/state fixtures and normalize ONLY generated IDs/timestamps via explicit map; preserve permissions/versions/business events/outcome uncertainty. Drive all named acceptance cases and journeys, inject crash at transactional/provider boundaries, retain results and failure evidence. The desktop is the Flutter application at apps/desktop and is not a Go import: transport parity here includes a Go wire-level driver that exercises exactly what that client consumes over the real internal/server listener (request/result envelope, original-key replay after disconnect, command.get for unknown acknowledgment, cursor expiry with snapshot recovery, mutual-TLS principal mapping), so named cases shared with the desktop prove the controller side of its contract. Dart unit/widget tests live in apps/desktop; driving the built application lives in qualification. Record dependency changes through designated root owner, not all agents. All spec-only tests must be labeled planned until actually executed.

Local proving focus: All embedded acceptance cases assigned integration, complete registry parity, three journeys each CLI/MCP plus interrupted cross-interface recovery; no skipped suite as pass.
...
**`internal/adapters/mcpclient`** — Qualified generic MCP client connection adapter (adapter name mcp; Postiz first).

Revision 4 governs MCP probe intent/admission/callback provenance, composed full parameter schemas, claim-time catalog/profile/session rechecks, bounded separately costed refusal replies and faithful no-response/pre-send evidence; see the shared contract. Adapter name mcp; profile schema zatiti.mcp/v1 has transport (streamable_http with one exact https endpoint and max_redirects 0; stdio frozen but capability_unsupported until qualified), protocol_version 2025-11-25|2026-07-28 (the server must negotiate the profile value), credential_kind bearer|none, allowed_tools, tool_call_cost, max_request_bytes, max_response_bytes, timeout_seconds, classifications and capability evidence. Action kind open_session/list_tools/call_tool/close_session per the frozen MCPClientParameters; the endpoint is the profile's, never an action field. Resolve Dispatch.CredentialRef through Secrets outside transactions and place the bytes only in the Authorization header; the staged request record, evidence, errors and logs carry no credential (credential-in-path and OAuth code flows are capability_unsupported). open_session performs the handshake exactly as the pinned SDK Client.Connect does (an optional SEP-2575 server/discover probe, then the legacy initialize and notifications/initialized; at most three recorded HTTP requests, every one listed in evidence.handshake, the declared primary-handshake exception; separately bounded refusal replies are additional recorded exchanges) and keeps the go-sdk ClientSession in a bounded in-memory table keyed by an opaque MCPSessionHandle; a handle unknown after restart or expiry is prerequisite_missing, never re-derived. list_tools is one tools/list request for one page; call_tool is exactly one tools/call after validating arguments strictly against the pinned input_schema whose digest the action carries and checking the tool against profile allowed_tools; close_session is one DELETE. Use the official MCP Go SDK pinned in dependencies.lock.json with SDK retries and reconnects disabled. Timeout after bytes were sent is outcome_unknown; a response above max_response_bytes is outcome_unknown with error_code response_oversize; Reconcile is capability_unsupported (the protocol has no authoritative call lookup) and unknown stays unknown until a separately admitted linked operation resolves it. Answer server-to-client requests (sampling, elicitation, roots, ping) only within the explicit action/profile control-reply budget, with method-not-found; record refusals and dispatched exchanges, abort and locally evict on exhaustion; open no GET listening stream; never call resources, prompts or tasks; never fetch resource_link or embedded resource content (a fetch is a separate governed read). Tool result content is staged as tool_result under the action's classification; annotations are recorded as hints and change nothing. No hidden retry, redirect, preflight or fallback endpoint.
...
Action: Dispatch one call_tool, one list_tools and one close_session attempt with zero control-reply allowance; then dispatch open_session against a fresh server. Repeat calls with explicit refusal-reply limits 1, 4 and 16 and excess server requests.

Expected:

- With zero control-reply allowance, call_tool, list_tools and close_session each produce exactly one primary HTTP request per claimed attempt; next_cursor is never followed by the adapter.
- open_session produces at most the three handshake requests the pinned SDK makes (server/discover probe, initialize, initialized), every one listed in evidence.handshake in order; no GET listening stream is opened.
- SDK retry and reconnect are disabled; a permitted retry is a new authorized recorded attempt.
- Explicit bounded method-not-found replies are the only additional POSTs: limits 1, 4 and 16 allow at most 2, 5 and 17 total call exchanges respectively. Exhaustion aborts and evicts locally without cleanup requests.
- Every dispatched reply has pre-send context and exchange evidence and consumes the declared per-reply cost bound; no hidden reply, retry or fabricated status is allowed.
...
### Z05.mcp_governed_probe_and_catalog — Z05

Setup: An unverified MCP connection binds an installed synthetic profile; ordinary effect policy and accounting are configured.

Action: Prepare validation through its public owner operation, attempt a cloned public effect, record a real attempt observation, discover tools, then change the catalog after admitting a tool call and before claiming it.

Expected:

- Only the exact durable owner intent permits initial validation without prior freshness; its job links the governed effect.
- Callbacks read the recorded observation for the exact operation and attempt; supplied JSON cannot invent validation or discovery truth.
- Discovered tools expose the full pinned adapter envelope and nonzero configured cost bound, with explicit endpoint and connection bindings.
- Claim rejects a changed catalog version before dispatch and does not consume the claim.
