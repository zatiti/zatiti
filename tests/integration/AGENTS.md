...
Setup: Pin the actual public Serenity commit/protocol and install its runtime/read facade with the Mac release on clean native Intel and Apple Silicon hosts.

Action: Prove scoped recall and write/curation, one canonical writer per brain, command disposition after lost acknowledgement, bounded spend/disclosure behavior, exportable backup revision and paused restore reconciliation through public interfaces.
